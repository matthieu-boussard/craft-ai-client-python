...
class TestGetDecisionTreesBulkSuccess(unittest.TestCase):
  ...
  def test_get_decision_trees_bulk_without_timestamp(self):
    ...
    """
    id1 = "yolo_ma_gueule"
    id2 = "amusement_de_fou"
    self.client.delete_agent(id1)
    self.client.create_agent(valid_data.VALID_CONFIGURATION, id1)
    print(valid_data.VALID_OPERATIONS_SET)
    self.client.add_operations(id1, valid_data.VALID_OPERATIONS_SET)

    self.client.delete_agent(id2)
    self.client.create_agent(valid_data.VALID_CONFIGURATION, id2)
    self.client.add_operations(id2, valid_data.VALID_OPERATIONS_SET)

    payload = [{"id": id1},
               {"id": id2}]
    decision_trees = self.client.get_decision_trees_bulk(payload)

    true_payload = [{"id": id1, "timestamp": 1458741230 + 505},
                    {"id": id2, "timestamp": 1458741230 + 505}]

    ground_truth_decision_tree = self.client.get_decision_trees_bulk(true_payload)
    self.assertEqual(decision_trees[0].get("tree").get("_version"),
                     ground_truth_decision_tree[0].get("tree").get("_version"))
    self.assertEqual(decision_trees[0].get("tree").get("configuration"),
                     ground_truth_decision_tree[0].get("tree").get("configuration"))

    #print(decision_trees[0])
    #print(ground_truth_decision_tree[0])

    self.assertEqual(decision_trees[0].get("tree").get("trees"),
                     ground_truth_decision_tree[0].get("tree").get("trees"))
