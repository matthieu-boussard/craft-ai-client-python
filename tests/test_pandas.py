...
def test_add_operations_df():
    ...
    assert_equal(
        agent["firstTimestamp"], SIMPLE_AGENT_DATA.first_valid_index().value // 10 ** 9
    )
    assert_equal(
        agent["lastTimestamp"], SIMPLE_AGENT_DATA.last_valid_index().value // 10 ** 9
    )
...
def test_add_operations_df_complex_agent():
    ...
    agent = CLIENT.get_agent(AGENT_ID)
    assert_equal(
        agent["firstTimestamp"], COMPLEX_AGENT_DATA.first_valid_index().value // 10 ** 9
    )
    assert_equal(
        agent["lastTimestamp"], COMPLEX_AGENT_DATA.last_valid_index().value // 10 ** 9
    )
...
def test_add_operations_df_missing_agent():
    ...
    agent = CLIENT.get_agent(AGENT_ID)
    assert_equal(
        agent["firstTimestamp"], MISSING_AGENT_DATA.first_valid_index().value // 10 ** 9
    )
    assert_equal(
        agent["lastTimestamp"], MISSING_AGENT_DATA.last_valid_index().value // 10 ** 9
    )
...
def test_get_operations_list_df():
    ...
    assert_equal(
        df.first_valid_index(), pd.Timestamp("2013-01-01 00:00:00", tz="Europe/Paris")
    )
    assert_equal(
        df.last_valid_index(), pd.Timestamp("2013-01-01 04:59:00", tz="Europe/Paris")
    )
...
def test_get_state_history_df():
    ...
    assert_equal(
        df.last_valid_index(), pd.Timestamp("2013-01-01 04:58:20", tz="Europe/Paris")
    )
...
def test_get_operations_list_df_complex_agent():
    ...
    assert_equal(
        df.last_valid_index(), pd.Timestamp("2013-01-10 00:00:00", tz="Europe/Paris")
    )
...
def test_decide_from_missing_contexts_df():
    ...
    assert_equal(
        df.last_valid_index(), pd.Timestamp("2013-01-02 00:00:00", tz="Europe/Paris")
    )
