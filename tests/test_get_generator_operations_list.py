...
def merge_sorted_operation_arrays_by_timestamp(operations_a, operations_b):
  ...
  (array_to_add_from, cursor_to_add_from) = (operations_a, cursor_a) \
...
class TestGetGeneratorOperationsListSuccess(unittest.TestCase):
  ...
  def setUp(self):
    ...
    expected_operations_2 = list(map(add_agent_name_2_to_operation, SMALL_VALID_OPERATIONS_SET))
  ...
  def test_get_generator_operations_list_with_lower_bound(self):
    ...
    self.assertIsInstance(ops, list)
    expected_ops = [op for op in self.expected_operations if op["timestamp"] >= lower_bound]
    self.assertEqual(ops, expected_ops)

  def test_get_generator_operations_list_with_upper_bound(self):
    upper_bound = 1462824549
    ops = self.client.get_generator_operations_list(self.generator_id, None, upper_bound)
    self.assertIsInstance(ops, list)
    expected_ops = [op for op in self.expected_operations if op["timestamp"] <= upper_bound]
    self.assertEqual(ops, expected_ops)

  def test_get_generator_operations_list_with_both_bounds(self):
    lower_bound = 1462824549
    upper_bound = 1464356844
    ops = self.client.get_generator_operations_list(self.generator_id, lower_bound, upper_bound)
    self.assertIsInstance(ops, list)
    expected_ops = [op for op in self.expected_operations if (op["timestamp"] >= lower_bound and
