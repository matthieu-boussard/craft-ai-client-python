...
class CraftEstimator(BaseEstimator):
  ...
  def fit(self, X, y):
    ...
    if self.add_timestamps_for_static:
      X = CraftEstimator.add_timestamp(X)
      y = CraftEstimator.add_timestamp(pd.DataFrame(y))

    # Add the output in the input dataframe - the craft ai client takes both in an unique DF.
    X = X.assign(**{self.output_name:y})
    ...
    self.craft_client.add_operations(self.agent_name, X)
  ...
  def predict(self, X):
    ...
    if self.add_timestamps_for_static:
      ...
      X = CraftEstimator.add_timestamp(X, last_timestamp)
    else:
      last_timestamp = int(X.index.values[0]) // 10**9
    ...
    # Compute the decision of this DT on the input data
    predictions_df = self.craft_client.decide_from_contexts_df(tree, X)
...
class CraftEstimatorClassifier(CraftEstimator, ClassifierMixin):
  def __init__(self,
               ...
               configuration=None,
               add_timestamps_for_static=False):
    super(CraftEstimatorRegressor, self).__init__(agent_name, configuration, add_timestamps_for_static)
